...
class TestFormatter(unittest.TestCase):
    ...
    def testBg(self):
        ...
        result = check_output(['echo', string])
        expected = str.encode(f'\x1b[48;2;250;165;0m{self.text}\x1b[0m\n')
    ...
    def testFg(self):
        ...
        result = check_output(['echo', string])
        expected = str.encode(f'\x1b[38;2;250;165;0m{self.text}\x1b[0m\n')
        self.assertEqual(result, expected)

    def testBold(self):
        string = self.formatter.bold().bake()
        result = check_output(['echo', string])
        expected = str.encode(f'\x1b[1m{self.text}\x1b[0m\n')
        self.assertEqual(result, expected)

    def testDim(self):
        string = self.formatter.dim().bake()
        result = check_output(['echo', string])
        expected = str.encode(f'\x1b[2m{self.text}\x1b[0m\n')
        self.assertEqual(result, expected)
    ...
    def testFlush2(self):
        ...
        result = check_output(['echo', string])
        expected = str.encode('\x1b[38;2;250;165;0mAnother string\x1b[0m\n')
        self.assertEqual(result, expected)

    def testFormatChain(self):
        string = self.formatter('Chain is cool!').bg('#9dc1e3').bold().fg('#1b8148').bake()
        result = check_output(['echo', string])
        expected = b'\x1b[48;2;157;193;227m\x1b[1m\x1b[38;2;27;129;72mChain is cool!\x1b[0m\n'
        self.assertEqual(result, expected)
